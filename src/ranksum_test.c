...
RSR_T *ranksum_from_stats(
   ...
   double ta_obs	/* sum of ranks of positives */
	...
	mww->p_twotailed = mww->p_right < mww->p_left ? 2*mww->p_right : 2*mww->p_left;
	mww->u = ua;
        //fprintf(stderr, "na= %d nb= %d R1 (ta_obs)= %f ta_max= %f U = %f\n", na, nb, ta_obs, ta_max, ua);
	return mww;
}
