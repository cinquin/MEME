...
/** Scaling of logo stack height **/
typedef enum {SHANNON, FREQUENCY} LOGOTYPE;
...
*/
static void LS_calc(ARRAY_T* items, ALPH_T* alph, size_t n, LOGOTYPE logotype, bool ssc, LSTACK_T* lstack) {
  int asize, col;
  ...
  stack_height = logotype == SHANNON ? LS_calc_R(items, asize, n, ssc) : 1;
  for (col = 0; col < asize; col++) {
...
static char* EPS_data_token(LP_PARAMS_T* params) {
  ...
  for (lnr = 0; lnr < params->l_num; lnr++) {
    ...
    for (pos = 0; pos < num_rows; pos++) {
      ...
      LS_calc(get_matrix_row(pos, line->mat), params->alphabet, line->snum, params->logotype, params->ssc, lstack);
...
static RBTREE_T* EPS_tokens(LP_PARAMS_T* params) {
  ...
  rbtree_put(tokens, "$BARBITS", params->logotype == SHANNON ? STR_dbl(log2(alph_size_core(params->alphabet))) : STR_dbl(1));
  ...
  // Label for y-axis
  rbtree_put(tokens, "$YAXISLABEL", (params->y_axis_label != NULL ? params->y_axis_label : params->logotype == SHANNON ? "bits" : "frequency"));
...
static void process_options(OPTIONS_T* o, int argc, char* argv[]) {
  ...
  INPUT_T *c;
  char *file;
  char *optstring = "-o:c:e:f:h:w:n:t:d:x:y:p:i::m:s:rlFSBEONXY";
  ...
  while (1) {
    ...
    switch (opt) {
      ...
      case 'F': p->logotype = FREQUENCY; break; // create a frequency logo
