...
>
      ...
      <body bgcolor='#D5F0FF'>  
        ...
        <table summary="buttons" align="left" cellspacing="0">
          <tr>
            ...
            <td bgcolor="#DDFFDD">
              <a href="#sec_i"><b>High-scoring Motif Occurrences</b></a>
            </td>
            <td bgcolor="#DDDDFF">
  ...
  <xsl:template name="high_scoring_motif_occurrences">
    ...
    <hr/>
    <center>
      <big><b><a name="sec_i">SECTION I: HIGH-SCORING MOTIF OCCURRENCES</a></b></big>
    </center>
    <hr/>
    <ul>
    <xsl:if test="count(/fimo/incomplete-motifs) &gt; 0">
      ...
      occurrences for each motif in memory.
      The following motifs each had more 
      than 
      <xsl:value-of select="/fimo/settings/setting[@name='max stored scores']"/> 
      occurrences with p-value less than
      <xsl:value-of select="/fimo/settings/setting[@name='output threshold']"/>.
      The less significant occurrences were dropped.
      <div style="padding-left: 0.75in; line-height: 1em; font-family: monospace;">
      <table>
        <thead>
          <tr>
      ...
      The maximum number of occurrences to be kept in memory can be set using the
    ...
    <xsl:if test="$match_count &gt; $max_match_count">
       ...
       matches are shown here.</b>
       The full set of motif occurrences can be seen in the
    ...
    <table border="1">
      ...
      <tbody>
        <xsl:for-each select="document('cisml.xml')/cis:cis-element-search/cis:pattern/cis:scanned-sequence/cis:matched-element[position() &lt;= $max_match_count]">
